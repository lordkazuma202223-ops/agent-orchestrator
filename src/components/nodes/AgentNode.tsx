...
export function NodeComponent({ data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`px-3 py-2 sm:px-4 sm:py-3 rounded-lg border-2 bg-slate-800 shadow-xl transition-all ${
        selected ? 'border-purple-500 ring-2 ring-purple-500/50' : 'border-slate-600'
      }`}
      style={{ minWidth: '240px', maxWidth: '300px' }}
    >
      {/* Handles for connections */}
      <Handle type="target" position={Position.Left} className="!bg-green-500 !w-3 !h-3 sm:!w-4 sm:!h-4" />
      <Handle type="source" position={Position.Right} className="!bg-blue-500 !w-3 !h-3 sm:!w-4 sm:!h-4" />
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-1">
          ...
          {isEditing ? (
            <input
              ...
              className="bg-slate-700 text-white px-2 py-1 rounded text-xs sm:text-sm w-full focus:outline-none focus:ring-2 focus:ring-purple-500"
              autoFocus
            />
          ) : (
            <span className="text-white font-semibold text-xs sm:text-sm">{nodeData.name}</span>
          )}
        </div>
        <button
          onClick={() => {
        ...
        {isEditing ? (
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <input
              ...
              className="bg-slate-700 text-white px-2 py-1 rounded w-12 sm:w-16 focus:outline-none focus:ring-2 focus:ring-purple-500 text-xs sm:text-xs"
            />
            <span>sec</span>
          </div>
        ) : (
